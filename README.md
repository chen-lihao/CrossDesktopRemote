...
> 当前状态：**M0 工程基线已完成，M1 Apple 与 M1B Windows 双向原型进行中。** iPad→Mac 基本连接、画面和远程输入已验证；Mac/Windows 进入共享角色后自动上线并取得一次性连接码。文件/剪贴板阶段 1～3 已完成协议、Rust 核心、桌面文本剪贴板和显式文件传输；阶段 4 已接入 iPad 用户触发式文本粘贴、系统文件选择、安全作用域暂存、应用内接收确认，以及“文件”导出和 Share Sheet。Windows/iPad 双机物理验收与后续文件剪贴板尚未完成。
...
Mac 选择“共享本机”后会自动上线并生成连接码；“在线等待”阶段只连接信令并发布 DNS-SD，不采集屏幕。先点击“设置远程输入权限”，iPad 的“附近设备”会通过 Bonjour 自动显示被控端，点击后只需输入相同六位连接码。未过期的正确连接码验证成功后才开始采集并建立会话，不再需要被控端二次允许。一段远程会话结束后，Mac 保持在线意图，自动生成新的单次连接码并恢复等待；只有点击“下线并停止接受连接”才退出。如果网络禁止 mDNS，仍可从“控制端可用连接地址”列表选择对应 Wi-Fi/有线地址手动输入。macOS 的屏幕录制和事件注入权限必须由本机用户在系统设置中授予；未授权时会话降级为仅观看。当前连接码信令只用于开发环境，不可暴露到公网。
...
| 模块 | 已通过 | 未通过或未完成 |
| --- | --- | --- |
| Flutter / Native | 响应式壳层、Apple纵向链路、Windows/macOS桌面直接IME、Windows原生全屏；桌面DNS-SD、统一跨平台切屏事务、非阻塞显示几何和输入路径；macOS/Windows文本剪贴板；Mac/Windows/iPad显式文件通道和传输中心；iPad用户触发式粘贴、`UIDocumentPickerViewController`安全暂存、接收后导出/分享；暂停/恢复/取消、`.cdrpart`原子落盘与会话内续传；`analyze`零告警、162项测试通过且1项按设计跳过，macOS/iOS Debug构建成功 | Windows MSVC构建、Windows/iPad各30次主副屏往返和Mac/Windows/iPad双机文件传输物理验收；20 GB、磁盘满、输入P95与应用重启后续传；随后实现文件剪贴板 |
