...
Future<void> _showExplicitFileTransferCenter(
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  session.explicitFileTransferReady
                      ? '选择文件或目录后，对方确认接收目录才会开始传输。'
                ...
                if (tasks.isEmpty)
                  ...
                  for (final task in tasks) ...[
                    _ExplicitFileTransferTaskTile(task: task, session: session),
...
class _ExplicitFileTransferTaskTile extends StatelessWidget {
  const _ExplicitFileTransferTaskTile({
    required this.task,
        ...
        child: Column(
          ...
          children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        '${task.state.label} · ${_formatTransferBytes(task.totalBytes)}',
                      ),
                    ],
                  ),
                ),
            ...
            if (task.progress case final progress?) ...[
              ...
              Text(
                '${_formatTransferBytes(task.transferredBytes)} / '
                '${_formatTransferBytes(task.totalBytes)}',
  ...
  Future<void> _acceptTransfer(BuildContext context) async {
    try {
      final destination = await getDirectoryPath(confirmButtonText: '保存到此处');
      if (destination == null) return;
      await session.acceptExplicitFileTransfer(task.id, destination);
      if (context.mounted) AppMessenger.show('已开始接收文件');
    } catch (error) {
      if (context.mounted) {
  ...
  Widget _buildToolbar(
    ...
    return ColoredBox(
      ...
      child: _RemoteToolbar(
        ...
        onFileTransfer: () =>
            _showExplicitFileTransferCenter(context, widget.session),
