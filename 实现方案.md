...
M1 Apple 原型先保持单路视频：Mac 枚举显示器并上报稳定 ID，iPad 选择时发送带递增序号的切换请求。macOS 13+ 在同一个 `SCStream` 上使用 `updateConfiguration()` 和 `updateContentFilter()` 原地切换显示器，整个会话保持同一个 RTC Track、`RTCRtpSender`、transceiver、MID、SSRC 和 DataChannel；项目内 WebRTC fork 在新几何首帧到达时更新视频源输出适配，并向应用发布只含来源 ID/尺寸的首帧状态。切换采用 `requested → captureReady → mediaFlowing → committed` 双端提交：Mac 验证目标首帧及 `framesEncoded` 增长，iPad 验证 `framesDecoded` 增长后更新显示器名称与远程输入目标。期望、编码和 Renderer 尺寸的差异属于后台画质收敛，不再触发回滚；只有目标源无帧、编码/解码完全不增长或连接失败才恢复原显示器。旧系统保留单 Sender `replaceTrack()` 兼容路径，但不重建 Sender、不重新协商 SDP。Texture 与指针映射共用唯一的居中内容矩形，输入始终绑定已经渲染的显示器。该方案消除了正常切屏的双路采集与尺寸误判，并避免连接状态和移动端 Texture 重建；多路独立 track、缩略图和平铺仍属于后续交付。
