...
iOS 软件键盘不再把 Flutter 输入框作为远程文档镜像，而是通过 Swift `UITextView` 输入代理接入 UIKit IME。`setMarkedText` 只维护本机组合态，`insertText`/`unmarkText` 发送最终 Unicode，`deleteBackward`、Return 和 Tab 发送独立可靠按键事件；每次提交后清空短期输入缓冲。Apple 简体拼音从拉丁拼音转换为不含拉丁尾串的完整 CJK 候选时立即提交，revision 门禁防止随后 `unmarkText` 重复发送。默认 UI 只显示组合长度，不保存已提交历史，因此被控端主动删除或移动光标不会反向破坏控制端输入状态。“本地编辑后发送”作为第三方输入法和特殊应用的兼容入口，由独立路由组件持有并销毁输入控制器，避免弹层退出动画期间访问已释放对象。Android 后续按同一 `RemoteImeInputAdapter` 接口使用原生 `InputConnection` 实现，不能复用远程文本差分算法。
...
M1 macOS 原型先固定可验证的 SDR 基线：项目内 `flutter_webrtc 1.6.0` fork 使用 ScreenCaptureKit `420v` Video-Range NV12，显式指定 ITU-R BT.709 色彩空间和 YCbCr Matrix，并向 PixelBuffer 传播 BT.709 primaries、transfer function 与 matrix；macOS 15+ 额外显式设置 `captureDynamicRange = SDR`。每条采集流记录一次实际像素格式、Range 和色彩附件，Mac 同时上报每块显示器的色彩空间与当前/潜在 EDR headroom。若物理 HDR A/B 仍证明系统 SDR 映射错误，再在自有 ScreenCaptureKit 适配器中加入 Metal/Core Image HDR→SDR tone mapping，不能用要求用户关闭 HDR 代替修复。
...
M1 Apple 原型先保持单路视频：Mac 枚举显示器并上报稳定 ID，iPad 选择后由 Host 捕获目标显示器并通过 `RTCRtpSender.replaceTrack()` 原位替换视频轨道。这样切屏不重新建立 PeerConnection，也避免 iPad 同时解码多路视频；多路独立 track、缩略图和平铺仍属于后续交付。

M1 清晰度切换先通过 `RTCRtpSender.setParameters()` 修改 `scaleResolutionDownBy`、`maxBitrate` 和 `maxFramerate`，提供自动、720p、1080p、2K 和原画档位，不修改被控端系统显示分辨率，也不重新建立会话。后续根据 WebRTC Stats 增加带迟滞的自动升降档，并在性能不足时再把目标尺寸前移到原生采集层。

控制端/被控端角色不能由操作系统硬编码。客户端以 `DeviceCapabilities` 描述平台能力：当前 macOS 同时开放控制和共享角色，iOS/iPadOS 只开放控制角色；Windows/Linux/Android 在原生采集与输入桥完成后再开启对应 Host 能力。当前先支持空闲状态下切换单一角色，后台 Host Service 与并行外连会话属于后续阶段。
...
3. 按 4MiB 分块，多文件/大文件最多 4 路并发；动态背压。
4. 每块带 BLAKE3 或 SHA-256 摘要，完整文件最终使用 SHA-256 校验。
5. 写入随机命名的 `.partial` 临时文件，完成校验后原子重命名；断线保存已完成块位图并支持续传。
...
- Java 只提供开发态六位连接码 WebSocket 信令：Host 先注册，连接码 5 分钟有效、单次消费、同一来源一分钟最多失败 5 次；正确连接码自动授权本次会话。macOS 屏幕录制和事件注入权限不能绕过，使用 `CGRequestPostEventAccess`/`CGPreflightPostEventAccess` 请求与复查，未授权时降级为仅观看。第一轮不实现账号、无人值守、文件、剪贴板、音频、同时多屏和公网服务。
- 将“用户希望持续共享本机”和“某一条 WebRTC 会话”拆为两层状态：已成功建立的会话断开后，Host 自动释放旧资源、生成新的单次连接码并恢复等待；只有用户显式点击“停止共享本机”才退出共享状态。首次连接失败或自动恢复再次失败时停止重试，避免无界重连循环。
- macOS 桌面窗口默认内容区为 `1180×760`，最小内容区为 `900×620`，确保首次打开即进入桌面响应式布局；小屏设备仍由 Flutter 断点和系统窗口约束处理。
- Apple 原型的色彩退出门禁包括：诊断值为 SDR/BT.709/Video-Range NV12；普通桌面和 HDR 开关 A/B 不明显泛白；灰阶 235～255 保持主要层次；主屏和扩展屏可独立保存/恢复显示调整。Metal tone mapping 只在显式 SDR 输出仍失败时启动。
