...
iOS 软件键盘不再把 Flutter 输入框作为远程文档镜像，而是通过 Swift `UITextView` 输入代理接入 UIKit IME。`setMarkedText` 只维护本机组合态，`insertText`/`unmarkText` 发送最终 Unicode，`deleteBackward`、Return 和 Tab 发送独立可靠按键事件；每次提交后清空短期输入缓冲。Apple 简体拼音从拉丁拼音转换为不含拉丁尾串的完整 CJK 候选时立即提交，revision 门禁防止随后 `unmarkText` 重复发送。默认 UI 只显示组合长度，不保存已提交历史，因此被控端主动删除或移动光标不会反向破坏控制端输入状态。“整句发送”作为第三方输入法和特殊应用的显式兜底。Android 后续按同一 `RemoteImeInputAdapter` 接口使用原生 `InputConnection` 实现，不能复用远程文本差分算法。
...
- 原型阶段使用 `flutter_webrtc` 1.6.x 验证 PeerConnection、ScreenCaptureKit 捕获、H.264、DataChannel 和 Apple 原生视频视图；视频帧不得进入 Dart 堆。版本必须锁定并纳入依赖/许可证审查。
