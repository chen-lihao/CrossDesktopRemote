...
所有协议消息使用 Protobuf，消息包含 `protocol_version`、`feature_bits`、`request_id` 和大小上限；未知字段可忽略，破坏性变更通过新消息类型或主版本处理。
...
| macOS | ScreenCaptureKit | CGEvent/Accessibility API | ScreenCaptureKit 是高性能、细粒度捕获路径；必须引导屏幕录制和辅助功能授权 |
...
- 部署最小 Java WebSocket 信令服务与 coturn，测试 LAN、普通 NAT、CGNAT/对称 NAT、UDP 禁用和网络切换；同时验证 1 万模拟空闲连接的心跳、清理和内存基线。
