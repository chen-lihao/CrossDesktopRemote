...
M1 Apple 原型先保持单路视频：Mac 枚举显示器并上报稳定 ID，iPad 选择时发送带递增序号的切换请求。macOS 13+ 在同一个 `SCStream` 上使用 `updateConfiguration()` 和 `updateContentFilter()` 原地切换显示器，整个会话保持同一个 RTC Track、`RTCRtpSender`、transceiver、MID、SSRC 和 DataChannel；更新期间暂停转发，完成后只接受 `.complete`、尺寸匹配且连续稳定的目标帧。切屏首批帧先在填充 Video-Range 黑色的新 NV12 画布上渲染，防止旧 IOSurface 未覆盖区域泄漏为 Sidecar 底栏残片。切换采用 `requested → captureReady → keyframeEncoded → keyframeDecoded → committed` 双端提交：Mac 在目标首帧稳定后通过项目内 WebRTC 编码器包装层请求本次 generation 的 H.264 关键帧，并验证 `keyFramesEncoded` 增长；iPad 仅在 `keyFramesDecoded` 增长后更新显示器名称与远程输入目标。关键帧请求允许一次有限重试，统计不可用或超时则保留/恢复原显示器，不把旧参考帧误判为新画面。iPad 在前 120ms 保留上一帧；仍未完成时才用不透明遮罩隔离旧 Texture，800ms 后显示明确等待状态，目标帧到达后立即以 120ms 淡出。遮罩不定时清空 Texture，避免目标帧已解码但事务统计尚未完成时误删静态画面。旧系统保留单 Sender `replaceTrack()` 兼容路径，但不重建 Sender、不重新协商 SDP。Texture 与指针映射共用唯一的居中内容矩形，输入始终绑定已经渲染的显示器。多路独立 track、缩略图和平铺仍属于后续交付。

M1 清晰度切换同时控制原生采集与 RTP Sender。项目内插件通过 `SCStream.updateConfiguration()` 调整采集目标长边和帧率，避免先采集 Retina/Sidecar 全尺寸再由编码器缩小；Sender 继续设置 `maxBitrate`、`maxFramerate` 和降级偏好，不修改被控端系统显示分辨率，也不重新建立会话。自动模式默认 1080p30/7 Mbps，以 1 秒区间统计观察丢包、RTT、可用出站带宽、编码耗时、掉帧、冻结和拥塞原因；连续 2 个坏样本降档，连续 10 个好样本且至少稳定 15 秒才升档，档位为 1080p60、1080p30、720p60、720p30。应用失败回滚到原档位；手动 720p、1080p、2K 和原画不受自动策略覆盖。macOS 编码器优先 VideoToolbox H.264；显示器切换只显式请求一次关键帧并允许一次重试，避免周期性大关键帧引发拥塞。
...
- 将“用户希望持续共享本机”和“某一条 WebRTC 会话”拆为两层状态：WebRTC 瞬时 `Disconnected` 先进入 8 秒恢复窗口，不释放资源或更换连接码；恢复超时或收到明确的 `hangup/peer-left` 后，Host 才释放旧资源、生成新的单次连接码并恢复等待。只有用户显式点击“停止共享本机”才退出共享状态。首次连接失败或自动恢复再次失败时停止重试，避免无界重连循环。
