...
阶段 1 已建立 `TransferTransport` 抽象、WebRTC 回调适配和窄 C ABI。当前桌面 MVP 受 `flutter_webrtc` DataChannel API 边界限制，网络分片编排暂位于独立 Dart 应用服务，SHA-256 位于工作 Isolate，文件字节不进入 Widget 树；生产门禁仍要求把磁盘数据泵与 no-follow 安全写入器下沉到 Rust/平台原生层。QUIC 后续实现同一传输接口，不改变任务状态机和上层协议。
...
桌面文件复制粘贴不再把远程文件物化到临时目录后写入操作系统文件剪贴板，也不在 OLE/File Promise 回调中发起网络下载。统一事务如下：

1. `Ctrl/Command+C` 只建立不可变 FileOffer，保存随机 Offer ID、剪贴板 revision、源路径快照和最长十分钟租约；协议只公布显示名称和数量，不发送本机绝对路径或文件内容。
2. `Ctrl/Command+V` 在目标端创建 PasteIntent，并通过平台适配器读取该时刻前台 Finder/Explorer 的活动目录。
3. 目标端生成一次性 DestinationLease；它保存规范化路径、平台目录身份和写权限快照，路径只保存在目标端内存，网络上只传 128 位随机租约 ID。
4. 源端收到有效请求后才调用显式文件传输引擎。目标端用 `destinationLeaseId` 接受任务并直接写入冻结目录；后续鼠标移动、窗口切换或打开其他目录不改变目标。
5. 接收任务开始前按 macOS device/inode 或 Windows volume/file-index 重新验证目录身份和写权限；SHA-256 校验和原子落盘完成后，目标端发送 Commit。
6. 整个事务使用 `sessionId + offerId + generation + pasteIntentId` 作为不可拆分身份，Commit 再绑定 `destinationLeaseId + transferId`。超时、拒绝、断线和取消都撤销租约，不注入第二次系统粘贴快捷键。
7. 本地剪贴板改为文字或其他文件时撤销尚未开始的 Offer；已经持有目标租约的传输独立完成，避免后续剪贴板变化把进行中的任务重定向。

能力使用 `destination-leased-file-paste-v1` 单独协商。旧客户端缺少能力时只隐藏复制粘贴文件语义，继续提供显式文件传输，不回退到已经废弃的系统文件剪贴板物化链路。Windows 使用只在 Explorer/桌面命中的低级键盘钩子，macOS 使用只在 Finder 命中的 CGEventTap；进程内多个会话通过所有权仲裁共享一个原生监听。iPadOS 不启用全局监听，继续使用用户触发的 Document Picker 和导出/分享。
...
- 被控端可要求标准隐私屏；只有全部活动显示器已覆盖且遮挡窗口已从采集中排除时才启动共享，拓扑变化导致覆盖失效时立即断开。
- 标准隐私屏不宣称覆盖 Windows UAC/Winlogon、macOS 登录窗口或其他用户会话；Windows 驱动级隐私模式必须使用签名服务、Credential Provider 与 Indirect Display Driver 单独实现。
...
## 18. 会话内核、视频策略、审计与窗口实现补充（2026-09-02）

本轮把页面状态中隐含的会话事实收口到四个边界：`RemoteSessionKernel`负责稳定`sessionId`和领域事件；Java信令服务负责连接码租约与到期主动推送；`RemoteVideoPolicy`独立表达分辨率、帧率、码率和降级偏好；`SessionAuditRepository`负责本地分页审计。文件任务以同一`sessionId`关联，不再由会话页反推控制器瞬时状态。

桌面画面采用单一renderer所有权：设备页只显示连接摘要和“打开远程桌面”，远程视频只在独立工作区挂载。启用Flutter windowing时，macOS/Windows/Linux使用同一isolate的原生窗口并由该窗口独立控制全屏；未启用或位于iPad/移动端时使用应用内全屏路由。关闭画面窗口不会断开PeerConnection、DataChannel、剪贴板或文件任务。

多显示器默认继续使用已物理验证的单窗口切屏事务。设置层已保留“每个显示器独立窗口”，但必须同时满足`multi-display-stream-v1`：每屏独立Sender/Transceiver、`mid → displayId`绑定、每窗口独立几何和输入displayId、最多两路及全局带宽预算。当前客户端不声明该能力，因此不会用一条活动视频轨道生成多个表面，也不会影响现有主副屏切换。

审计库使用SQLite进行十条一页的稳定游标分页；设备、地址、信令服务器、显示器、结果及文件源/目标路径封装进AES-GCM payload，主密钥位于系统安全存储。数据库只保留分页所需ID、时间和状态列明文，不保存屏幕像素、剪贴板内容或键盘输入。

## 19. 远程视频展示绑定契约（2026-09-08）

远程媒体与展示面通过不可变的 `RemoteVideoBinding` 连接，字段固定为 `MediaStream stream`、真实 `trackId` 和单调递增 `generation`。`RTCPeerConnection.onTrack` 是绑定身份的唯一来源；展示层不得只传 Stream 让平台自行猜测默认轨道，也不得使用 `"0"`、数组下标或其他哨兵值伪装 Track ID。显示器切换保持同一个 Sender/Track 时，generation 用于隔离旧展示回调，而不是触发重建 PeerConnection。

项目内 `flutter_webrtc` fork 的 `videoRendererSetSrcObject` 必须在 Windows、Darwin 和 Android 遵守相同契约：先确认 Renderer 和 Stream 存在，再从该 Stream 中精确查找请求的 Track，绑定成功后返回实际 `trackId`；任一对象缺失都返回结构化错误，不能记录日志后仍返回成功。Dart 层只有收到成功确认后才能更新 `srcObject`，避免会话状态与原生 Texture 实际状态分裂。

`RemotePresentationController` 独立拥有 Renderer 和 Texture 生命周期，状态为 `detached → attaching → surfaceReady → binding → waitingFirstFrame → ready/visible`。绑定成功不等于用户已看到画面；三秒内必须同时收到首帧通知和非零渲染尺寸，否则进入带错误原因和重试入口的 `failed` 状态。窗口关闭只卸载展示面，网络会话、输入、剪贴板和文件任务仍由会话层持有。

画面修复采用非破坏事务：控制端先读取 `inbound-rtp` 帧计数；媒体仍推进时只重新验证同一 `stream + trackId + generation` 绑定，不解绑 Texture、不重建 Track；媒体停滞或统计不可用时才请求 Host 关键帧，Windows Host 仅在关键帧请求不受支持时热重启当前采集 Track。任何失败保留原会话并进入可重试状态，禁止把“刷新”实现为无条件 `setSrcObject(null)`。
