...
连接前会读取当前 input desktop 和活动显示器。处于锁屏、登录或 UAC 安全桌面时，会话保持在“等待被控端可交互”状态，最多等待三分钟；回到用户 `Default` desktop 后自动继续。该等待机制不会把普通应用伪装成安全桌面控制：需要远程输入登录凭据的无人值守能力仍必须使用独立 Windows Service、每用户 Agent 和 Credential Provider。
...
- 对 CrossDesktopRemote 窗口连续执行最小化/恢复、最大化/还原、拖动改尺寸各 30 次；这些管理窗口事件不得重建 Desktop Capture 轨道，远端画面和输入持续推进。
- 远程点击 CrossDesktopRemote 自身关闭按钮只收起管理窗口，不终止承载采集和输入的进程；Windows 本机点击关闭仍保持系统原生语义。
