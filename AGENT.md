...
仓库当前已完成 M0 工程基线，并进入 M1 iPad→Mac 局域网原型。开发态连接码已有 5 分钟 TTL、单次消费和每来源限流，验证后自动建立 WebRTC；动态比例/全屏、触控板/直接触控、系统/快捷双键盘、Mac 权限状态、单 `SCStream` 多显示器切换、关键帧双端提交、事件驱动遮罩、区间媒体诊断和带滞回自动画质已编码。macOS、iOS 和物理 iPad 构建通过。真实双设备输入、连续切屏、自动升降档、多屏热插拔和稳定性仍需人工验收，不得声称完整远控或生产安全已经完成。
