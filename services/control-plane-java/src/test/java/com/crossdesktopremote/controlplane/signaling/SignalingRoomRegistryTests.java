...
class SignalingRoomRegistryTests {
	...
	void expiresAHostRegistrationAndConsumesAValidCodeOnce() {
		...
		var registry = new SignalingRoomRegistry(now::get, Duration.ofMinutes(5), Duration.ofMinutes(1), 5);
	...
	@Test
	void rateLimitsRepeatedInvalidControllerCodesPerAddress() {
		var now = new AtomicLong(1_000);
		var registry = new SignalingRoomRegistry(now::get, Duration.ofMinutes(5), Duration.ofMinutes(1), 5);
		...
		assertThat(registry.join("999999", SignalingRole.CONTROLLER, controller))
				.isEqualTo(SignalingRoomRegistry.JoinResult.RATE_LIMITED);
